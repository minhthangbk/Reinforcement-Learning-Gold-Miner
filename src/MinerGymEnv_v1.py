...
from Config import *
from MinerEnv_v1 import MinerEnv_v1 as MinerEnv
from MinerEnv_v1 import TreeID, TrapID, SwampID
import cv2
...
# THANG check feasibility for each step
    def check_topleftcorner(self):
        return ((self.state.x == 0) and (self.state.y == 0))

    def check_toprightcorner(self):
        return ((self.state.x == self.state.mapInfo.max_x) and (self.state.y == 0))

    def check_bottomleftcorner(self):
        return ((self.state.x == 0) and (self.state.y == self.state.mapInfo.max_y))
